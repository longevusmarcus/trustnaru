...
const constructScenePrompts = (careerPath: any, existingImageCount: number = 0): string[] => {
  ...
  return [
    // 1) Professional Scene - working as the role
    `HYPERREALISTIC shot of a ${roleTitle} actively working, demonstrating ${keySkills}. Shot on Canon EOS R5, 50mm f/1.4 lens, natural window lighting, shallow depth of field. CRITICAL: Use EXACT person from reference photo - identical facial features, skin texture, hair, body type, proportions. NO modifications to face or body. Photojournalistic style capturing authentic work moment. Work environment with depth and detail.`,
